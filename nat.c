...
void *packet_processing(void *args){
		...
		if ((ntohl(t_struct->iph->saddr) & local_mask) == (ntohl(internalIP) & local_mask)) {  // outbound traffic
			...
			if (tcph->syn == 1){
				...
				if (search_table_source(source_addr, source_port) == -1){
					entry_info = insert_entry(source_addr, source_port, publicIP);
			...
			tcph->source = htons(entry_info.translated_src_port);
	...
	int test = nfq_set_verdict(t_struct->qh, t_struct->id, NF_ACCEPT, (t_struct->data_len+t_struct->iph->ihl<<2), (unsigned char*)t_struct->iph);
